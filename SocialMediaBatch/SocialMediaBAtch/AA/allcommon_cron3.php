                ...
                while ($row = mysql_fetch_array($keyword_result)) {
                    ...
                    if (!empty($row['name'])) {
                        ...
                        if ($search_data->errorCode == '0' && !empty($search_data->message)) {
                            $err_user = array('project_id' => $project_id, 'user_auto_id' => $user_auto_id);
...
if(YOUTUBECRONSTART == '1'){
        ...
        while ($row = mysql_fetch_array($result)) {
            if (!empty($row['name'])) {
                ...
                $project_ids_arr[] = $row['keyword_auto_id'];
            }
        }
        #FETCH ALL KEYWORDS [END]
        $total_project_ids = count($project_ids_arr);

        if ($total_project_ids) {
            for ($i = 0; $i < $total_project_ids; $i++) {

                $project_id = $project_ids_arr[$i];
                                        $keyword_name = $keywords_arr[$i];

                #CHECK CRON ALREADY EXECUTED TODAY? [START]

                $where = array('project_id' => $project_id, 'updated_date' => date('Y-m-d'));
                if (getRowCount('tbl_youtube_cron_log', $where) > 0) {
                    ...
                    $youtube_cron_log_data['project_id'] = $project_id;
                ...
                if ($updated_date < date('Y-m-d')) {
                    ...
                    if (!empty($entries) && $video_info_result_count == 0) {
                        ...
                        foreach ($entries as $ek => $ev) {
                            ...
                            $where = array('project_id' => $project_id, 'video_id' => $video_id);
                            ...
                            foreach ($comments_entry as $ck => $cv) {
                                ...
                                $youtube_comment_data['project_id'] = $project_id;
...
if(GOOGLEPLUSCRONSTART == '1'){
            ...
            $sql = "SELECT * FROM tbl_keywords where id='11'"; // where id='92'
            ...
            while ($row = mysql_fetch_array($result)) {
                ...
                $sql1 = "SELECT * FROM tbl_googleplus_info where project_id = '".$project_id."'";
                $result1 = mysql_query($sql1) or die(mysql_error());
                $count1 = mysql_num_rows($result1);

               if($count1 > "0"){
                    //update code
                    $sql18 = "DELETE FROM tbl_googleplus_info where project_id = '".$project_id."'";
                    $result18 = mysql_query($sql18) or die(mysql_error());
                    $sql19 = "DELETE FROM tbl_googleplus_comments where project_id = '".$project_id."'";
                    ...
                    if(!empty($finalData->items)){
                        foreach ($finalData->items as $data){
                            ...
                            $sql23 = "INSERT INTO  `tbl_googleplus_info` (   `displayname` ,  `userid`, `title` ,  `content` ,  `plus1count` ,  `commentcount` ,  `resharescount` ,  `objecttype` ,  `url` ,  `publish` ,  `project_id` ) 
                                        VALUES (
                                ...
                                if(!empty($finalDataComment->items)){
                                    foreach ($finalDataComment->items as $data){
                                        ...
                                        if(!empty($data->object->content)){
                                            $sql33 = "INSERT INTO `tbl_googleplus_comments` (`displayname`,`userid`, `content`,`objecttype`,`publish`,`project_id`) 
                                                          VALUES ('".remove_spacial_1(remove_spacial(clean_insert($data->actor->displayName)))."',
                                                                                '0',
                                                                                '".((clean_insert($data->object->content)))."',
                                                                                '".$data->object->objectType."',
                                                                                '".date("Y-m-d H:i:s", strtotime($data->published))."',
                                                                                '".$project_id."')";
                    ...
                    if(!empty($finalData->error)){
                        $sql25 = "INSERT INTO  `tbl_error_log` (reason,message,type,project_id) 
                                            values ('".$finalData->error->errors->reason."','".$finalData->error->errors->message."','googleplus','".$project_id."')";
                   ...
                   //'".clean_insert($data->title)))))."', 
                        foreach ($finalData->items as $data){
                                    $sql23 = "INSERT INTO  `tbl_googleplus_info` 
                                                                    (   `displayname` ,  `userid`, `title` ,  `content` ,  `plus1count` ,  `commentcount` ,  `resharescount` ,  `objecttype` ,  `url` ,  `publish` ,  `project_id` ) 
                                                                                                                                                            VALUES (
                                                                                                                                                                    '".remove_spacial_1(remove_spacial(clean_insert($data->actor->displayName)))."',
                                                                                                                                                                    '0',
                                                                                                                                                                    '".remove_spacial_1(remove_spacial(clean_insert($data->title)))."',
                                                ...
                                                //echo $sql23."<br/>";
                                    mysql_query($sql23) or die(mysql_error()."==========>7=====>".$keywordName."====".$project_id);
                                    $lastInsertId = mysql_insert_id();
                                    if($data->object->replies->totalItems != "0"){
                                        $url = "https://www.googleapis.com/plus/v1/activities/".$data->id."/comments?key=".GOOGLEPLUSKEY;
                                        //$output_json1 = _curl($url);
                    ...
                    if(!empty($finalData->error)){
                        $sql24 = "INSERT INTO  `tbl_error_log` (reason,message,type,project_id) 
                                            values ('".$finalData->error->errors->reason."','".$finalData->error->errors->message."','googleplus','".$project_id."')";
